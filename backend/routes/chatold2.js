...
// POST /api/chat/:requestId/recommendation/draft — doctor only, AI-assisted
// starting point for the care plan. Doctor can edit before sending.
router.post('/:requestId/recommendation/draft', async (req, res, next) => {
  try {
    ...
    const request = await loadAuthorizedRequest(req, res);
    if (!request) return;
    if (request.status !== 'completed') return res.status(409).json({ error: 'The session must end before drafting a recommendation' });

    const allMessages = await ChatMessage.find({ request: request._id }).sort({ createdAt: 1 });
    ...
    const { data } = await axios.post(
      'https://api.groq.com/openai/v1/chat/completions',
      {
        ...
        messages: [
          {
            role: 'system',
            content: `You are helping a doctor draft a short "Recommendation / Care Plan" for a patient after a live chat about digital or nicotine addiction recovery. This is explicitly NOT a medical prescription — never mention drug names, dosages, or medical directives. Write 3-5 short, warm, actionable bullet points (habits to try, things to watch for, a suggested next step) based on the conversation. Plain text, no markdown symbols, no preamble — the doctor will read and edit this before sending.`,
          },
          { role: 'user', content: transcript || 'No messages were exchanged in this session.' },
        ],
        max_tokens: 300,
        temperature: 0.6,
      },
...
// POST /api/chat/:requestId/recommendation — doctor only, saves + delivers the care plan
router.post('/:requestId/recommendation', async (req, res, next) => {
  try {
    ...
    const request = await loadAuthorizedRequest(req, res);
    if (!request) return;
    if (request.status !== 'completed') return res.status(409).json({ error: 'The session must end before sending a recommendation' });
    ...
    try {
      await triggerEvent(request._id.toString(), 'recommendation-sent', request.recommendation);
    } catch { /* non-fatal — patient will see it next time they load the dashboard */ }
