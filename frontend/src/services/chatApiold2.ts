...
export interface ChatRoomInfo {
  request: {
    ...
    recommendation?: { content: string; sentAt: string } | null;
  };
...
export async function draftRecommendation(requestId: string, token: string): Promise<string> {
  const data = await request<{ draft: string }>(`/chat/${requestId}/recommendation/draft`, token, { method: 'POST' });
  return data.draft;
}

export async function sendRecommendation(requestId: string, token: string, content: string): Promise<{ content: string; sentAt: string }> {
  const data = await request<{ recommendation: { content: string; sentAt: string } }>(
    `/chat/${requestId}/recommendation`, token, { method: 'POST', body: JSON.stringify({ content }) }
  );
