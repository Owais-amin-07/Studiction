...
export interface ChatRoomInfo {
  request: {
    ...
    recommendation?: { content: string; sentAt: string | null } | null;
  };
...
export async function sendRecommendation(requestId: string, token: string, content: string): Promise<void> {
  await request(`/chat/${requestId}/recommendation`, token, {
    method: 'POST',
    body:   JSON.stringify({ content }),
  });
}
