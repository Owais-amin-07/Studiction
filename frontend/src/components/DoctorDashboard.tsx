...
import {
  ...
  Calendar, Clock, FileText, MessageSquare, Sparkles,
} from 'lucide-react';
import * as doctorApi from '../services/doctorApi';
import type { DoctorData, DoctorFacingRequest } from '../services/doctorApi';
...
const PLACEHOLDER_SECTIONS = [
  ...
  { icon: FileText,     title: 'Patient Reports Archive',note: 'Past session reports will be searchable here.' },
];

const URGENCY_COLOR: Record<string, string> = { low: '#48cfad', moderate: '#f0b429', high: '#ef4444' };
...
export default function DoctorDashboard({ doctor, onLogout }: DoctorDashboardProps) {
  ...
  const [requests, setRequests]           = useState<DoctorFacingRequest[]>([]);
  const [activeSessions, setActiveSessions] = useState<DoctorFacingRequest[]>([]);
  ...
  const loadRequests = async () => {
    try {
      const list = await doctorApi.getPendingRequests();
      setRequests(list);
  ...
  const handleAccept = async (id: string) => {
    setAcceptingId(id);
    try {
      const accepted = await doctorApi.acceptRequest(id);
      setRequests((prev) => prev.filter((r) => r.id !== id));
      setActiveSessions((prev) => [...prev, accepted]);
    } catch (err: any) {
      setRequestsError(err.message || 'Could not accept this request');
    } finally {
      setAcceptingId(null);
    }
  };
  ...
  // Full-screen takeover while a chat is open — same pattern as any other view swap in this app.
  if (openChatRequestId) {
    const token = doctorApi.getDoctorToken();
    if (token) {
      return (
        <LiveChat
          ...
          token={token}
          onExit={() => {
            setActiveSessions((prev) => prev.filter((r) => r.id !== openChatRequestId));
  ...
  return (
    <div className="relative min-h-screen w-full px-4 md:px-8 py-8 max-w-5xl mx-auto">
      ...
      {/* Pending Requests — real data, not a placeholder */}
      <motion.div
        initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }}
        className="glass-panel rounded-3xl p-6 mb-6"
        style={{ boxShadow: '0 0 40px rgba(240,180,41,0.05)' }}
      >
        <div className="flex items-center justify-between mb-4">
          <p className="text-white text-sm font-semibold">Pending Requests</p>
          <button type="button" onClick={loadRequests} className="text-zinc-500 hover:text-zinc-300 transition-colors cursor-pointer">
            <RefreshCw className={`w-3.5 h-3.5 ${loadingRequests ? 'animate-spin' : ''}`} />
          </button>
        </div>

        {requestsError && <p className="text-rose-400 text-xs mb-3">{requestsError}</p>}

        {loadingRequests ? (
          <div className="flex justify-center py-8"><RefreshCw className="animate-spin w-5 h-5 text-zinc-600" /></div>
        ) : requests.length === 0 ? (
          <p className="text-zinc-600 text-xs py-2">No pending requests right now — new ones will show up here automatically.</p>
        ) : (
          <div className="space-y-3">
            <AnimatePresence>
              {requests.map((req) => (
                <motion.div
                  ...
                  style={{ borderColor: 'rgba(255,255,255,0.06)', background: 'rgba(6,6,17,0.5)' }}
                >
                  <div className="flex items-start justify-between gap-3 mb-2">
                    <div className="flex items-center gap-2">
                      <Sparkles className="w-3.5 h-3.5" style={{ color: '#f0b429' }} />
                      <p className="text-white text-sm font-semibold">{req.patientName}</p>
                    </div>
                    <span className="text-[9px] font-bold uppercase tracking-widest px-2 py-0.5 rounded-full flex-shrink-0"
                      style={{ color: URGENCY_COLOR[req.report.urgency], background: `${URGENCY_COLOR[req.report.urgency]}20` }}>
                      {req.report.urgency} urgency
                    </span>
                  </div>
                  <p className="text-zinc-400 text-xs mb-1">{req.report.mainConcern}{req.report.keyDetails ? ` · ${req.report.keyDetails}` : ''}</p>
                  <p className="text-zinc-500 text-xs leading-relaxed mb-3">{req.report.summary}</p>

                  {(() => {
                    const msLeft = new Date(req.createdAt).getTime() + 12 * 3600 * 1000 - Date.now();
                    if (msLeft <= 0) return null;
                    const h = Math.floor(msLeft / 3600000);
                    const m = Math.floor((msLeft % 3600000) / 60000);
                    return <p className="text-[10px] text-zinc-600 mb-2">⏳ Expires in {h}h {m}m</p>;
                  })()}

                  <button
                    type="button" onClick={() => handleAccept(req.id)} disabled={acceptingId === req.id}
                    className="flex items-center gap-1.5 h-8 px-3.5 rounded-lg text-white text-xs font-bold uppercase tracking-wider cursor-pointer disabled:opacity-50"
                    style={{ background: 'linear-gradient(135deg, #f0b429, #f59e0b)' }}
                  >
                    {acceptingId === req.id ? <RefreshCw className="animate-spin w-3.5 h-3.5" /> : <Check className="w-3.5 h-3.5" />}
                    Accept
                  </button>
                </motion.div>
              ))}
      ...
      {/* Active Sessions — accepted requests, ready to chat */}
      {activeSessions.length > 0 && (
        <motion.div
          initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }}
          className="glass-panel rounded-3xl p-6 mb-6"
          style={{ boxShadow: '0 0 40px rgba(72,207,173,0.05)' }}
        >
          <p className="text-white text-sm font-semibold mb-4">Active Sessions</p>
          <div className="space-y-3">
            {activeSessions.map((session) => (
              <div key={session.id} className="flex items-center justify-between gap-3 rounded-2xl p-4 border"
                style={{ borderColor: 'rgba(72,207,173,0.25)', background: 'rgba(6,6,17,0.5)' }}>
                <div className="flex items-center gap-2">
                  <MessageSquare className="w-4 h-4" style={{ color: '#48cfad' }} />
                  <p className="text-white text-sm font-semibold">{session.patientName}</p>
                </div>
                <button
                  type="button" onClick={() => setOpenChatRequestId(session.id)}
                  className="h-8 px-3.5 rounded-lg text-white text-xs font-bold uppercase tracking-wider cursor-pointer"
                  style={{ background: 'linear-gradient(135deg, #48cfad, #3ab595)' }}
                >
                  Open Chat
                </button>
              </div>
