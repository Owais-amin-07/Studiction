...
export interface SessionConfig {
  ...
  mood: 'robotic';
  isPremiumFlow?: boolean; // true when routed through the (now unlockable) Real Doctor card
}

interface SessionSetupProps {
  onStart: (config: SessionConfig) => void;
  isPremium?: boolean;
  onRequirePremium?: () => void; // navigates to the Premium purchase flow
}
...
export default function SessionSetup({ onStart, isPremium = false, onRequirePremium }: SessionSetupProps) {
  ...
  const [selectedMood, setSelectedMood] = useState<'ai' | 'doctor'>('ai');

  const handleSelectDoctor = () => {
    if (!isPremium) { onRequirePremium?.(); return; }
    setSelectedMood('doctor');
    setSelectedMinutes(20); // premium sessions are fixed at 20 — locks the duration picker below too
  };

  const isDoctorFlow = selectedMood === 'doctor';
      ...
      >
        ...
        <motion.div variants={itemVariants} className="mb-6">
          <div className="flex items-center justify-between px-1 mb-2">
            <label className="text-zinc-400 text-[11px] font-bold uppercase tracking-widest block">
              How much time do you have?
            </label>
            {isDoctorFlow && (
              <span className="text-[10px] font-medium" style={{ color: '#f0b429' }}>
                Locked at 20 min with your doctor
              </span>
            )}
          </div>
          <div className="grid grid-cols-2 gap-2.5">
            {DURATIONS.map(({ minutes, label, icon: Icon }) => {
              ...
              const isDisabled = isDoctorFlow && minutes !== 20;
              return (
                <motion.button
                  key={minutes}
                  type="button"
                  disabled={isDisabled}
                  onClick={() => !isDoctorFlow && setSelectedMinutes(minutes)}
                  ...
                  style={{
                    borderColor: isSelected ? (isDoctorFlow ? 'rgba(240,180,41,0.6)' : 'rgba(108,99,255,0.6)') : 'rgba(255,255,255,0.06)',
                    background:  isSelected ? (isDoctorFlow ? 'rgba(240,180,41,0.08)' : 'rgba(108,99,255,0.08)') : 'rgba(6,6,17,0.6)',
                    boxShadow:   isSelected ? `0 0 20px ${isDoctorFlow ? 'rgba(240,180,41,0.15)' : 'rgba(108,99,255,0.15)'}` : 'none',
                    opacity:     isDisabled ? 0.35 : 1,
                    cursor:      isDisabled ? 'not-allowed' : 'pointer',
                  }}
                >
                  <Icon className="w-4 h-4" style={{ color: isSelected ? (isDoctorFlow ? '#f0b429' : '#6c63ff') : '#71717a' }} />
                  <div>
        ...
        <motion.div variants={itemVariants} className="mb-7">
          ...
          <div className="grid grid-cols-2 gap-2.5">
            <motion.button
              type="button"
              onClick={() => setSelectedMood('ai')}
              whileHover={{ scale: 1.02 }}
              whileTap={{ scale: 0.98 }}
              className="relative flex flex-col items-start gap-1.5 p-3.5 rounded-xl border text-left cursor-pointer transition-all duration-200"
              style={{
                borderColor: !isDoctorFlow ? 'rgba(72,207,173,0.5)' : 'rgba(255,255,255,0.06)',
                background:  !isDoctorFlow ? 'rgba(72,207,173,0.06)' : 'rgba(6,6,17,0.6)',
                boxShadow:   !isDoctorFlow ? '0 0 20px rgba(72,207,173,0.12)' : 'none',
              }}
            >
              <Cpu className="w-4 h-4" style={{ color: !isDoctorFlow ? '#48cfad' : '#71717a' }} />
              <div>
            ...
            {/* Real Doctor — unlockable once Premium is active */}
            <motion.button
              type="button"
              ...
              className="relative flex flex-col items-start gap-1.5 p-3.5 rounded-xl border text-left cursor-pointer transition-all duration-200"
              style={{
                borderColor: isDoctorFlow ? 'rgba(240,180,41,0.6)' : 'rgba(255,255,255,0.06)',
                background:  isDoctorFlow ? 'rgba(240,180,41,0.08)' : 'rgba(6,6,17,0.6)',
                boxShadow:   isDoctorFlow ? '0 0 20px rgba(240,180,41,0.15)' : 'none',
                opacity:     !isPremium ? 0.6 : 1,
              }}
              title={isPremium ? 'Connect with a real doctor' : 'Requires Premium'}
            >
              <Stethoscope className="w-4 h-4" style={{ color: isDoctorFlow ? '#f0b429' : '#71717a' }} />
              <div>
                <p className="text-white text-sm font-semibold">Real Doctor</p>
                <p className="text-zinc-500 text-[11px]">{isPremium ? 'Live handoff after a quick chat' : 'Requires Premium'}</p>
              </div>
              {isDoctorFlow ? (
                <span className="absolute top-2.5 right-2.5 text-[9px] font-bold uppercase tracking-widest px-1.5 py-0.5 rounded-full"
                ...
                <span className="absolute top-2.5 right-2.5 flex items-center gap-1 text-[9px] font-bold uppercase tracking-widest px-1.5 py-0.5 rounded-full text-zinc-400"
                  ...
                  {!isPremium && <Lock className="w-2.5 h-2.5" />} Premium
                </span>
              )}
            </motion.button>
          </div>
        ...
        <motion.div variants={itemVariants}>
          <motion.button
            type="button"
            onClick={() => onStart({ durationMinutes: selectedMinutes, mood: 'robotic', isPremiumFlow: isDoctorFlow })}
            whileHover={{ scale: 1.02 }}
            whileTap={{ scale: 0.98 }}
            className="w-full h-11 rounded-xl text-white font-semibold tracking-wider uppercase text-[13px] transition-all cursor-pointer flex items-center justify-center gap-1.5"
            style={{
              background: isDoctorFlow
                ? 'linear-gradient(135deg, #f0b429, #f59e0b)'
                : 'linear-gradient(135deg, #6c63ff, #48cfad)',
              boxShadow: isDoctorFlow
                ? '0 0 25px rgba(240,180,41,0.22)'
                : '0 0 25px rgba(108,99,255,0.22)',
            }}
          >
            {isDoctorFlow ? 'Begin Premium Intake' : 'Begin Session'} <ChevronRight className="w-4 h-4" />
          </motion.button>
        </motion.div>
      </motion.div>
    </div>
