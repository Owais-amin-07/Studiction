...
import { Send, Clock, PlusCircle, PhoneOff, Sparkles, RefreshCw, Stethoscope, User as UserIcon } from 'lucide-react';
...
export default function LiveChat({ requestId, role, token, onExit }: LiveChatProps) {
  ...
  const [recommendation, setRecommendation] = useState<{ content: string; sentAt: string } | null>(null);
  ...
  useEffect(() => {
    ...
    chatApi.getChatRoom(requestId, token)
      .then((data) => {
        ...
        if (data.request.recommendation) setRecommendation(data.request.recommendation);
    ...
    channel.bind('recommendation-sent', (payload: { content: string; sentAt: string }) => setRecommendation(payload));
  ...
  const handleDraftRecommendation = async () => {
    ...
    try {
      ...
      setError(err.message || 'Could not generate a draft right now');
    } finally {
      setIsDrafting(false);
    }
  };
  ...
  const handleSendRecommendation = async () => {
    ...
    try {
      setRecommendation(await chatApi.sendRecommendation(requestId, token, recDraft.trim()));
    } catch (err: any) {
      setError(err.message || 'Could not send the recommendation');
    } finally {
      ...
      <div className="flex-1 flex flex-col max-w-3xl mx-auto w-full px-4 py-6">
        ...
        {sessionEnded ? (
          <div className="glass-panel rounded-2xl p-5">
            <p className="text-white text-sm font-medium mb-1 text-center">This session has ended</p>

            {isDoctor ? (
              recommendation ? (
                <div className="mt-3">
                  <p className="text-zinc-500 text-[10px] uppercase tracking-widest font-bold mb-2">Recommendation sent</p>
                  <p className="text-zinc-300 text-xs leading-relaxed whitespace-pre-wrap rounded-lg p-3 mb-4"
                    style={{ background: 'rgba(240,180,41,0.06)' }}>{recommendation.content}</p>
                </div>
              ) : (
                <div className="mt-3 space-y-2">
                  <p className="text-zinc-500 text-xs text-center mb-2">Write a short recommendation / care plan for this member.</p>
                  <textarea
                    value={recDraft} onChange={(e) => setRecDraft(e.target.value)} rows={5}
                    placeholder="e.g. Keep a short daily log of urges and what triggered them..."
                    className="w-full px-3 py-2 bg-[#060611]/90 border border-[rgba(255,255,255,0.06)] focus:border-[#f0b429]/70 rounded-lg text-white placeholder-zinc-600 text-sm outline-none transition-colors resize-none"
                  />
                  <div className="flex gap-2">
                    <button type="button" onClick={handleDraftRecommendation} disabled={isDrafting}
                      className="flex-1 flex items-center justify-center gap-1.5 h-9 rounded-lg border text-zinc-300 hover:text-white text-xs font-medium cursor-pointer disabled:opacity-50"
                      style={{ borderColor: 'rgba(255,255,255,0.08)' }}>
                      {isDrafting ? <RefreshCw className="animate-spin w-3.5 h-3.5" /> : <Sparkles className="w-3.5 h-3.5" style={{ color: '#f0b429' }} />}
                      AI Draft
                    </button>
                    <button type="button" onClick={handleSendRecommendation} disabled={isSendingRec || !recDraft.trim()}
                      className="flex-1 h-9 rounded-lg text-white text-xs font-bold uppercase tracking-wider cursor-pointer disabled:opacity-40"
                      style={{ background: 'linear-gradient(135deg, #f0b429, #f59e0b)' }}>
                      {isSendingRec ? 'Sending...' : 'Send'}
                    </button>
                  </div>
                </div>
              )
              ...
              <div className="mt-3">
                <p className="text-zinc-500 text-[10px] uppercase tracking-widest font-bold mb-2">Your care plan</p>
                <p className="text-zinc-300 text-xs leading-relaxed whitespace-pre-wrap rounded-lg p-3 mb-4"
                  style={{ background: 'rgba(240,180,41,0.06)' }}>{recommendation.content}</p>
              </div>
            ) : (
              <p className="text-zinc-500 text-xs text-center mb-4 mt-1">Your doctor is preparing a recommendation — it'll appear here.</p>
            )}

            <button type="button" onClick={onExit}
              className="h-10 px-5 rounded-xl text-white text-xs font-bold uppercase tracking-wider cursor-pointer block mx-auto"
              style={{ background: isDoctor && !recommendation ? 'rgba(255,255,255,0.06)' : 'linear-gradient(135deg, #f0b429, #f59e0b)' }}>
              Back to Dashboard
            </button>
          </div>
        ) : (
