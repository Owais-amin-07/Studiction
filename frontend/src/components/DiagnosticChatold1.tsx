...
export default function DiagnosticChat({ onComplete, durationMinutes = 10, mood = 'robotic' }: DiagnosticChatProps) {
  const targetExchanges = targetExchangesFor(durationMinutes);
    ...
    <div className="flex flex-col h-screen w-full relative">
      ...
      >
        ...
        <div className="flex items-center gap-3">
          ...
          <div>
            <p className="text-xs text-zinc-300 font-semibold tracking-wide">{mood === 'doctor' ? 'Sage' : 'Studiction AI'}</p>
            <p className="text-[10px] text-zinc-600 uppercase tracking-widest">{mood === 'doctor' ? 'Recovery Companion' : 'Structured Assessment'}</p>
          </div>
        </div>
