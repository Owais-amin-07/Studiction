        ...
        {sessionEnded ? (
          <div className="glass-panel rounded-2xl p-5 text-center">
            ...
            <p className="text-zinc-500 text-xs mb-4">
              {isDoctor ? 'You can prepare their recommendation now.' : 'Your doctor will send a recommendation to your dashboard soon.'}
            </p>
            <button type="button" onClick={onExit}
              className="h-10 px-5 rounded-xl text-white text-xs font-bold uppercase tracking-wider cursor-pointer"
              style={{ background: 'linear-gradient(135deg, #f0b429, #f59e0b)' }}>
              {isDoctor ? 'Back to Dashboard' : 'Back to Dashboard'}
            </button>
          </div>
        ) : (
